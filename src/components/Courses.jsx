...
const Courses = () => {
    ...
    return (
        <div className="container mx-auto h-[90dvh] flex flex-col gap-5">
            {selectedCourse ? (
                <>
                    <button className='bg-c_1 text-white hover:text-black border-2 border-c_1 rounded-md hover:bg-opacity-15 hover:bg-black transition-all duration-200 ease-in p-2 w-fit self-center' onClick={() => setSelectedCourse(null)}>Return to courses</button>
                        ...
                        <div className='bg-c_5 p-5 flex flex-col shadow-md'>
                            ...
                            <div className="flex flex-wrap overflow-auto h-[35dvh] max-sm:h-[20dvh] justify-start">
                                {courses.filter(course => course.semester === selectedSemester).map((course) => (
                                    <div key={course.id} className="p-4 border flex justify-center items-center rounded-lg hover:bg-black bg-white transition-all duration-200 hover:bg-opacity-10  shadow-md m-2 cursor-pointer" onClick={() => setSelectedCourse(course.name)}>
